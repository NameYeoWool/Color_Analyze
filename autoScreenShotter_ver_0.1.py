...
import json
import requests
import Retangle_detect_gallery as Rec_Aalyze
...
class GUI:
    def __init__(self, master):
        ...
        self.nameLabel =tk.Label(master,text="PC방 이름 : ")


        # self.userInput_PcRoomName = tk.StringVar()
        #
        # self.PcRoomName=""
        # try:
        #     f = open("pcRoom_name.txt",'r')
        #     line = f.readline()
        #     self.PcRoomName = line.strip()
        #     print(self.PcRoomName)
        #     f.close()
        # except Exception as e:
        #     print(e)
        # self.showPcRoomName = tk.Label(master, text="Pc방 이름 :%s"%self.PcRoomName)
        # self.nameTxtBox = tk.Entry(master,textvariable=self.userInput_PcRoomName)
        # self.nameBtn =tk.Button(master,text="입력", command= [GUI.setName(self,self.userInput_PcRoomName.get()),
        #                                                     GUI.setShowName(self,self.userInput_PcRoomName.get())])
        ...
        self.startBtn = tk.Button(master,width=20 ,text="시작", command=self.switchon)
        self.endBtn = tk.Button(master, width=20 ,text="중지", command=self.switchoff)
        ...
        self.cntLabel = tk.Label(master, text=" 좌석 개수 :")

        self.userInput_saet_cnt = tk.StringVar()
        self.seat_cnt = "0"
        try:
            f = open("seat_cnt.txt",'r')
            line = f.readline()
            self.seat_cnt = line.strip()
            print(self.seat_cnt)
            f.close()
        except Exception as e:
            print(e)
            print(self.seat_cnt)
        self.showCntLabel = tk.Label(master, text="전체 좌석 개수 :%s"%self.seat_cnt)

        self.txtBox = tk.Entry(master,width=8 ,textvariable=self.userInput_saet_cnt)
        # action_with_arg = partial( GUI.setCnt(userInput), userInput)
        self.getCntBtn = tk.Button(master,width=8, text=" 입력",command = lambda :[GUI.setCnt(self,self.userInput_saet_cnt.get())
                                                                                , GUI.setShowCnt(self,self.userInput_saet_cnt.get())])
        ...
        # self.nameTxtBox.grid(row=0,column=2)
        # self.nameBtn.grid(row=0, column=3)
        # self.showPcRoomName.grid(row=1,columns=2)
        ...
        # coords = tk.StringVar()
        self.status.grid(row=6,column=1,rowspan=3,columnspan=3)
        self.startBtn.grid(row=10,column=0,columnspan=3)
        self.endBtn.grid(row=11,column=0,columnspan=3)
        #
        self.cntLabel.grid(row=13,column=1)
        self.txtBox.grid(row=13,column=2)
        self.getCntBtn.grid(row=13,column=3)

        self.showCntLabel.grid(row=15,column=1,columnspan=2)
        # #
        # self.setBtn.grid(row=7,column=1)
        # self.resetBtn.grid(row=8,column=1)
        self.areaBtn.grid(row=18,column=2)
        # self.areaLabel.grid(row=10,column=1)
        # # self.cntLabel.grid(row=11,column=1)
        # # self.setLabel.grid(row=11,column=1)
        # self.helpBtn.grid(row=12,column=1)
        ...
        #
        # self.cntLabel.pack(side=tk.LEFT,anchor=tk.N)
        # self.txtBox.pack(fill=tk.BOTH,padx=5,pady=5,anchor=tk.N)
        # # self.txtBox.place(x=110,y=415)
        #
    ...
    # def setShowName(self, input):
    #     self.showPcRoomName.config(text="PC방 이름 : %s"%input)
    #
    # def setName(self, userInput):
    #     f = open("pcRoom_name.txt", "w+")
    #     f.write("%s"%userInput)
    #     f.close()

    def setShowCnt(self, input):
        self.showCntLabel.config(text="전체 좌석 개수 : %s"%input)
    def setCnt(self, userInput):
        f = open("seat_cnt.txt", "w+")
        f.write("%s"%userInput)
        f.close()
        ...
        def run():
            ...
            if (setSeat == False):
                ...
                try:
                    ...
                    main_return = Rec_Aalyze.main()
                    # detectAppendSeatStatus(seatPosition,fullImage_height,fullImage_width, seat_height, seat_width)
                    cnt_seat, cnt_avail, cnt_unavail = Rec_Aalyze.detectAppendSeatStatus(main_return[0], main_return[1],
                    ...
                    Rec_Aalyze.drawSeat(main_return[0], main_return[1], main_return[2], main_return[3], main_return[4])
            ...
            # After seat Count set
                ...
                while (switch == True):
                    ...
                    try:
                        ...
                        main_return = Rec_Aalyze.main()
                        # detectAppendSeatStatus(seatPosition,fullImage_height,fullImage_width, seat_height, seat_width)
                        cnt_seat, cnt_avail, cnt_unavail = Rec_Aalyze.detectAppendSeatStatus(main_return[0],
                                                                                             main_return[1],
                                                                                             main_return[2],
                                                                                             main_return[3],
                                                                                             main_return[4])
                        ...
                        # draw the seat layout and fill the color
                            ...
                            if not pre_seat_position:  # empty
                                ...
                                Rec_Aalyze.drawSeat(main_return[0], main_return[1], main_return[2], main_return[3],
                                                    main_return[4])
                            ...
                            # check whether seat status are changed or not
                                ...
                                f = open("log.txt", "a+")
                                f.write(
                                    datetime.now().strftime('%Y-%m-%d %H:%M:%S') + " check pre and now SeatPosition \n")
                                f.close()
                                status = 1
